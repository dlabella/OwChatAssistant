...
namespace AntiToxicOverlay
{
    ...
    public class OverlayForm : Form
    {
        ...
        private void DrawChatDetected(Graphics g)
        {
            Rectangle indicatorRect = new Rectangle(
                ...
                using var brush = new LinearGradientBrush(
                    indicatorRect,
                    Color.FromArgb(220, 0, 60, 120),
                    Color.FromArgb(180, 0, 140, 255),
                    0f);

                FillChamfered(g, brush, indicatorRect, 8);

                using var pen = new Pen(
                    Color.FromArgb(220, 0, 200, 255), 2);

                DrawChamfered(g, pen, indicatorRect, 8);
                ...
                SizeF textSize =
                    g.MeasureString(text, fontSmall);

                float tx =
                    indicatorRect.X +
                    (indicatorRect.Width - textSize.Width) / 2;

                float ty =
                    indicatorRect.Y +
                    (indicatorRect.Height - textSize.Height) / 2;
        ...
        // =========================
        // 🔷 CHAMFERED SHAPE (no curves)
        // =========================

        private void FillChamfered(Graphics g, Brush b, Rectangle r, int cut)
        {
            using var path = ChamferedRect(r, cut);
            g.FillPath(b, path);
        }

        private void DrawChamfered(Graphics g, Pen p, Rectangle r, int cut)
        {
            using var path = ChamferedRect(r, cut);
            g.DrawPath(p, path);
        }

        private GraphicsPath ChamferedRect(Rectangle bounds, int cut)
        {
